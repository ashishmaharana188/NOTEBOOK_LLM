...
export function NotesDataProvider({
  ...
  const [notesByGroup, setNotesByGroup] = useState<Record<string, NoteItem[]>>(
    {},
  );
  ...
  const fetchNotesForGroupInternal = useCallback(
    async (
      ...
      try {
        const res = await axios.get(`${API_BASE}/notes/item/${groupId}`);
        if (
          fetchNotesReqByGroup.current[groupId] === currentReq &&
          res.data.status === "success"
        ) {
        ...
        if (fetchNotesReqByGroup.current[groupId] === currentReq) {
          setLoadingByScope((prev) => ({ ...prev, [scopeId]: false }));
        }
      }
    },
    [],
  ...
  const createStack = useCallback(
    async (title: string) => {
      try {
        const res = await axios.post(`${API_BASE}/notes/stacks/create`, { title });
  ...
  const deleteStack = useCallback(
    async (stackId: string) => {
      try {
        const res = await axios.delete(`${API_BASE}/notes/stacks/${stackId}`);
        if (res.data.status === "success") {
  ...
  const createGroup = useCallback(
    async (title: string, stackId: string) => {
      try {
        const res = await axios.post(`${API_BASE}/notes/groups/create`, {
          title,
          stack_id: stackId,
        });
        if (res.data.status === "success") {
  ...
  const deleteGroup = useCallback(
    async (groupId: string) => {
      try {
        const res = await axios.delete(`${API_BASE}/notes/groups/${groupId}`);
        if (res.data.status === "success") {
  ...
  const renameStack = useCallback(
    async (stackId: string, newTitle: string) => {
      setStacks((prev) =>
        prev.map((stack) =>
          stack.stack_id === stackId ? { ...stack, title: newTitle } : stack,
        ),
      );
      try {
  ...
  const renameGroup = useCallback(
    async (groupId: string, newTitle: string) => {
      setGroups((prev) =>
        prev.map((group) =>
          group.group_id === groupId ? { ...group, title: newTitle } : group,
        ),
      );
      try {
  ...
  const deleteNote = useCallback(
    async (
      ...
      try {
        const res = await axios.delete(`${API_BASE}/notes/item/${noteId}`);
        if (res.data.status === "success" && groupId) {
          await fetchNotesForGroupInternal(groupId, scopeId);
        }
      } catch (error) {
...
export default function useNotes(scopeId: NotesScopeId = DEFAULT_SCOPE_ID) {
  ...
  return {
    ...
    createNote: (
      ...
      context.createNote(groupId, title, content, tags, linkedEchoId, scopeId),
    updateNote: (
      noteId: string,
      groupId: string,
      title: string,

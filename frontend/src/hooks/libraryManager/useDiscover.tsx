...
export default function useDiscover() {
  ...
  const handleSearch = async (overridePage?: number) => {
    ...
    try {
      ...
      if (searchSource === "Gutenberg") {
        ...
        const params: any = { query: simpleQuery, page: currentPage, limit };
      ...
      const res = await ensureRolesThen(["embedding"], () =>
        API.get("/discover/search_v2", {
          params: {
            ...
            limit: limit,
            subject: selectedGenre !== "All" ? selectedGenre : undefined,
          },
        }),
      );
      if (!res) return;
  ...
  const handleDownload = async (book: DiscoveryResult, format: string) => {
    const endpoint =
      book.source === "Gutenberg" ? "/gutenberg/download" : "/ia/download";
    const payload =
      book.source === "Gutenberg"
        ? { book_id: book.id, title: book.title, preferred_format: format }
        : { identifier: book.id, title: book.title, preferred_format: format };
  ...
  const handleDirectIngest = async (book: DiscoveryResult, format: string) => {
    const endpoint =
      book.source === "Gutenberg" ? "/gutenberg/ingest-direct" : "/ia/ingest";
    const payload =
      book.source === "Gutenberg"
        ? {
            book_id: book.id,
    ...
    try {
      ...
      await ensureRolesThen(["embedding"], () => API.post(endpoint, payload));
    } catch (err) {

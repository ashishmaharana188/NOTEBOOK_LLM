...
function toLocationValue(raw: unknown): string | number | null {
  ...
  if (!Number.isNaN(numeric) && raw.trim() !== "" && String(numeric) === raw) {
    return numeric;
  }
  return raw;
}
...
export function useReaderSession(book: ReaderBook | null) {
  ...
  const [manifest, setManifest] = useState<ReaderManifestSummary | null>(null);
  ...
  const refreshBootstrap = useCallback(async () => {
    ...
    try {
      ...
      if (!hasHydratedLocationRef.current) {
        ...
        currentLocationRef.current = lastLocation !== null
          ? {
              location: lastLocation,
              locationType: payload.session?.last_location_type || "",
              progressPercent: payload.session?.progress_percent || 0,
      ...
      if (isTextFormat) {
        const initialSection = Math.max(
          0,
          Number(toLocationValue(payload.session?.last_location) || 0),
        );
  ...
  const flushSession = useCallback(async () => {
    if (!book?.filename || !sessionDirtyRef.current || !currentLocationRef.current) {
      return;
    }
      ...
      try {
        ...
        if (payload?.manifest) {
          setManifest((prev) => ({
            ...
            ...payload.manifest,
            status: payload.manifest_status || payload.manifest.status || "ready",
          }));
        }
  ...
  const updateAnnotation = useCallback(
    async (
      ...
      patch: Partial<Pick<
        ReaderAnnotation,
        "anchor" | "quote_text" | "title" | "note" | "color" | "kind" | "page_label" | "chapter_label"
      >>,
    ) => {
      ...
      try {
        const response = await API.put(`/reader/annotations/${annotationId}`, {
          ...
          chapter_label: patch.chapter_label ?? existing.chapter_label,
        });
  ...
  const deleteAnnotation = useCallback(async (annotationId: string) => {
    try {
      ...
      setAnnotations((prev) =>
        prev.filter((annotation) => annotation.annotation_id !== annotationId),
      );
    } catch (error) {
  ...
  const jumpToAnnotation = useCallback((annotation: ReaderAnnotation) => {
    const anchor = annotation.anchor || {};
    const locationValue = toLocationValue(
      anchor.location ?? anchor.section_index ?? annotation.page_label,
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cachedRaw) {
        const cached = JSON.parse(cachedRaw) as Partial<ReaderBootstrapPayload>;
        if (cached.manifest) setManifest(cached.manifest);
        if (cached.session) setSession(cached.session);
        if (Array.isArray(cached.annotations)) setAnnotations(cached.annotations);
        const cachedLocation = toLocationValue(cached.session?.last_location);
    ...
    if (book.initialReaderBootstrap) {
      ...
      setAnnotations(Array.isArray(bootstrap.annotations) ? bootstrap.annotations : []);
      const initialLocation = toLocationValue(bootstrap.session?.last_location);
      ...
      if (isTextFormat) {
        setCurrentTextSection(
          Math.max(0, Number(toLocationValue(bootstrap.session?.last_location) || 0)),
        );
      }
  ...
  useEffect(() => {
    if (!book?.filename || !manifest || manifest.status !== "building") return;
    if (bootstrapPollRef.current) window.clearInterval(bootstrapPollRef.current);
    ...
    reportLocation({
      ...
      progressPercent:
        manifest.section_index.length > 0
          ? ((currentTextSection + 1) / manifest.section_index.length) * 100
          : 0,

...
export function useEpubControl(
  ...
  const getRendition = (rendition: any) => {
    ...
    // 2. Inject styles into any contents already loaded
    ...
    // 3. Listen for Hooks (New content loading)
    rendition.hooks.content.register((content: any) => {
      injectStyle(content);
      attachActivityListeners(content);
    });

    // 4. SELECTION LISTENER (Restored & Fixed)
    // We filter by length to avoid the "instant partial selection" bug.
    rendition.on("selected", (cfiRange: string, contents: any) => {
      const range = rendition.getRange(cfiRange);
      const text = range ? range.toString() : "";

      // Only trigger if the selection is meaningful (> 2 words)
      // This prevents single-click jitters from firing a search.
      if (text && text.trim().split(/\s+/).length > 2) {
        console.log("📖 EPUB Selection:", text);
  ...
  // --- HELPER: Container Background Sync ---
  const getContainerBg = () => {
    if (settings.theme === "dark") return "#111827";

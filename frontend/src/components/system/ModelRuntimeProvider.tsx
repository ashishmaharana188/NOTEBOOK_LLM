...
interface RuntimeContextValue {
  ...
  loadRoles: (roles: RuntimeRole[], allowStartManaged?: boolean) => Promise<any>;
...
const getRuntimeData = (response: any) => response?.data?.data || response?.data || null;
...
export function ModelRuntimeProvider({
    ...
    socketRef.current.onmessage = (event) => {
      try {
        ...
        if (
          ...
          notify({
            title: "Runtime Error",
            message: data.message || "Model runtime operation failed.",
            tone: "error",
          });
        }
      } catch (error) {
        console.error("Failed to parse runtime websocket message", error);
      }
    };
  ...
  const isRolePrepared = useCallback(
    (role: RuntimeRole) =>
      Boolean(runtime?.roles?.[role]?.enabled || runtime?.roles?.[role]?.loaded),
    [runtime],
  );
  ...
  const runRuntimeAction = useCallback(
    async (
      ...
      try {
        ...
        setRuntime(data);
        if (successMessage) {
          notify({ title: successTitle, message: successMessage, tone: "success" });
        }
        return data;
      } catch (error: any) {
        const payload = extractPayload(error);
        notify({
          title: failureTitle,
          message: payload?.message || error?.message || "Operation failed.",
          tone: "error",
        });
        throw error;
      }
  ...
  const unloadRoles = useCallback(
    async (roles: RuntimeRole[]) =>
      runRuntimeAction(
        API.post("/system/models/unload", { roles, allow_start_managed: false }),
        "Models Unloaded",
        "Model Unload Failed",
    ...
    async <T,>(
      ...
      try {
        ...
        if (isModelLoadRequired(error)) {
          const payload = extractPayload(error);
          const requiredRoles = (payload?.required_roles || uniqueRoles) as RuntimeRole[];
          const confirmed = await confirmAction({
            title: options?.promptTitle || "Load Models",
            message:
              payload?.message ||
...
export function useModelRuntime() {
  ...
  if (!context) {
    throw new Error("useModelRuntime must be used within a ModelRuntimeProvider");
  }
  return context;
}

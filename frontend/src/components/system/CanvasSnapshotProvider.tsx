...
export function CanvasSnapshotProvider({
  ...
  const refreshCanvasSnapshot = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get("http://127.0.0.1:8000/brain/echoes/saved");
      if (currentReq === fetchReq.current && res.data.status === "success") {
        const normalizedClusters = (res.data.data || []).map((cluster: any) => ({
          ...cluster,

...
export function ManualCardCreator({
  ...
  const handleSpawn = async () => {
    ...
    try {
      ...
      const noteRes = await axios.post(buildApiUrl("/notes/item/create"), {
      ...
      if (noteRes.data.status === "success") {
        ...
        const newLayout = [...targetLayout, { type: "note", id: newNoteId }];

        // 2. Inject pointer into the Master Binder (Echo Mode)
        if (shouldInjectToEchoLayout) {
          await axios.put(buildApiUrl("/brain/cluster/layout"), {
    ...
    <div className="relative flex items-center border-l border-slate-700/50 pl-4 ml-2">
      {isOpen && (
        <div
          ...
          className="absolute bottom-[130%] right-0 mb-2 bg-slate-900/95  -xl border border-slate-700/50 shadow-2xl rounded-2xl p-5 w-[320px] z-[999999]"
        >

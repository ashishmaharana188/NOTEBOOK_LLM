  ...
  ({
    ...
    const { archiveChildren, previewItems } = useMemo(() => {
      ...
      const sorted = [...children].sort((a: any, b: any) => {
        ...
        if (orientA === "landscape" && orientB !== "landscape") return -1;
        if (orientB === "landscape" && orientA !== "landscape") return 1;
        return 0;
      });
      return { archiveChildren: children, previewItems: sorted.slice(0, 4) };
    }, [item.chunks, spatialMetadata]);

    const [localPosState, setLocalPosState] = useState({ x: baseX, y: baseY });
    ...
    const handleUpdateTitle = async (newTitle: string) => {
      ...
      try {
        await fetch("http://127.0.0.1:8000/brain/archive/update_title", {
          method: "PUT",
    ...
    const getArchiveTargetIdFromPointer = React.useCallback((event: any) => {
      const clientX =
      ...
      <motion.div
        ...
        style={{
          zIndex:
            ...
            savedMeta?.z_index ||
            (isSelected ? 9999 : 10),
          outline: isSelected && !isFannedOut ? "2px solid #4ade80" : "none",
          outlineOffset: "8px",
          borderRadius: "16px",
        ...
        {isVisible && (
          <div className="relative">
                  ...
                  previewItems.map((child: any, idx: number) => {
                    ...
                    return (
                      <BinderCoverPreview
                        key={idx}
                        title={
                          child.title || child.book_id || "Archived Content"
                        }
                        coverMedia={child.cover_media || child.cover_image}
            ...
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
              <AnimatePresence>
                {isFannedOut &&
                  archiveChildren.map((child: any, idx: number) => {
                    const childId =
                      canvasMode === "ECHO" ? child.id : child.stack_id;
                    const meta = spatialMetadata[childId] || {};
                    const isChildSelected =
                      resolvedSelectedItemIdSet.has(childId);
                    const isHovered = hoveredCardId === childId;
                    ...
                    return (
                      <motion.div
                        ...
                        style={{ zIndex: isHovered ? 60 : 40 - idx }}
                        // THE FIX: Mouse event listeners for the hover state
                        onMouseEnter={() => setHoveredCardId(childId)}
                        onMouseLeave={() => setHoveredCardId(null)}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (onSelect) onSelect(childId);
                        }}
                        onDragEnd={(event) => {
                          if (!isMergeMode || !onAppendToArchive) return;
                          ...
                          onAppendToArchive(
                            targetArchiveId,
                            isBulk ? selectedItemIds : [childId],
                          );
                        }}
                        onDoubleClick={(e) => {
                          e.stopPropagation();
                                    ...
                                    .map((innerItem: any, idx: number) => (
                                      ...
                                      >
                                        <h4 className="font-bold text-slate-700 text-xs leading-snug line-clamp-2">
                                          {innerItem.title ||
                                            innerItem.bridge ||
                                            innerItem.text?.substring(0, 30) ||
                                            "Untitled Note"}
                                        </h4>
                                      </li>
                                    ))}
                                  {!(
                                    child.chunks && child.chunks.length > 0
                                  ) && (

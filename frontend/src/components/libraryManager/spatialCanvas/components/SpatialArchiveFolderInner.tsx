...
const SpatialFolderCard = React.memo(
  ({
    ...
    const folderId = folder.chunk_id || folder.group_id || `folder-${index}`;
    ...
    const handleUpdateTitle = async (newTitle: string) => {
      ...
      try {
        await fetch("http://127.0.0.1:8000/notes/groups/update", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ group_id: folderId, title: newTitle }),
        });
      } catch (e) {
    ...
    const handleDeleteFolder = async () => {
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/brain/archive/inner/${folderId}`,
          {
            method: "DELETE",
          },
        );
    ...
    return (
      ...
      <motion.div
        ref={cardRef}
        data-selection-id={folder.chunk_id || folder.group_id || folder.id}
      ...
      >
        ...
        <motion.div
          className={`absolute inset-0 flex flex-col rounded-2xl transition-shadow duration-200 ${
            isSelected
        ...
        >
          <ArchiveFolderCard
            ...
            title={localTitle}
            count={
              folder.chunks?.length || folder.preview_items?.length || 0
            }
          ...
          >
            {previewItems.slice(0, 4).map((child: any, idx: number) => {
              let mediaUrl =
                child.cover_media ||
                child.cover_image ||
                child.image_url ||
              ...
              if (!mediaUrl && (child.text || child.content)) {
                const imgMatch = (child.text || child.content).match(
                  /<img[^>]+src="([^">]+)"/,
                );
              ...
              return (
                <BinderCoverPreview
                  ...
                  title={child.title || child.bridge || "Untitled"}
                  coverMedia={mediaUrl}

    ...
    useEffect(() => {
      ...
      if (isVault) {
        ...
        if (
          !activeFolder ||
          String(
            activeFolder.stack_id || activeFolder.id || activeFolder.group_id,
          ) !== String(binderId)
        ) {
        ...
        if (depth === 3) {
          ...
          if (
            !activeNode ||
            String(activeNode.group_id || activeNode.id) !== String(slotId)
          ) {
        ...
        if (
          ...
          const folder = allGroups?.find(
            (g: any) => String(g.group_id) === String(targetGroupId),
          );
          if (folder) {
            setActiveFolder(folder);
            if (fetchNotesForGroup) fetchNotesForGroup(targetGroupId);
          }
        }
      }
    }, [
    ...
    const handleCardSetActive = useCallback(
      (node: any) => {
        if (
          ...
          if (onDrillDown) onDrillDown(node.chunk_id || node.group_id);
          return;
        }
    ...
    return (
      <motion.div
        ...
        onDragEnd={(event, info) => {
          if (isMergeMode) {
            const targetArchiveId = getArchiveTargetIdFromPointer(event);
            if (targetArchiveId && onAppendToArchive) {
              const isBulk =
                selectedItemIds?.includes(itemId) && selectedItemIds.length > 1;
              onAppendToArchive(
                targetArchiveId,
          ...
          <>
            {!isExpanded && (
              <div className="absolute top-0 left-0 pointer-events-none group">
                {(() => {
                  ...
                  const currentLayerSet = zLayers[stackRotation] ?? [30, 20, 10];
                  const [coverZ, bg1Z, bg2Z] = currentLayerSet;
                  const isStackSelected = selectedItemIdSet.has(itemId);
                            ...
                            >
                              {item.is_folder ? (
                                <SpatialFolderCard
                                  ...
                                  total={2}
                                  previewItems={
                                    item.preview_items || item.chunks || []
                                  }
                                  isExpanded={false}
                                  isActive={false}
                                  enginePos={{ x: 0, y: 0 }}
            ...
            <div className="relative">
              {allItemsToRender.map((item: any, index: number) => {
                ...
                const finalCardLayout = currentLayoutMap[cardId];
                const slotEnginePos = finalCardLayout
                  ? { x: finalCardLayout.x, y: finalCardLayout.y }
                  : { x: 0, y: 0 };

                const finalZIndex =
                  finalCardLayout?.z ?? allItemsToRender.length - index;
                ...
                if (item.is_folder) {
                  ...
                  return (
                    <SpatialFolderCard
                      ...
                      onInteract={() => bringCardToFront(cardId)}
                      onExpand={() => {
                        setManualSlots({});
                        setPageIndex(0);
                        if (onDrillDown) onDrillDown(item.chunk_id);
                      }}
                      isSelected={selectedItemIdSet.has(cardId)}
                      ...
                      onDragEndCard={(
                        ...
                        if (isBulk) {
                          setManualSlots((prev: any) => {
                            const next = { ...prev };
                            selectedItemIds.forEach((id: string) => {
                              const originalPos = currentLayoutMap[id];
                              if (originalPos)
                                next[id] = {
                          ...
                          setLocalZOrder((prev: string[]) => {
                            const clean = prev.filter(
                              (id) => !selectedItemIdSet.has(id),
                            );
                            return [...clean, ...selectedItemIds];
                          });
                        } else {
                          setManualSlots((prev: any) => ({
                            ...prev,
                ...
                return (
                  <SpatialCard
                    ...
                    onUpdateStickies={handleTagsUpdateStable}
                    ...
                    cardZIndex={finalZIndex}
                    onInteract={() => bringCardToFront(cardId)}
                    linkSummary={linkSummaryByItemId?.[cardId]}
                    ...
                    onDragEndCard={(
                      ...
                      if (isMergeMode) {
                        const targetArchiveId = getArchiveTargetIdFromPointer(e);
                        if (targetArchiveId && onAppendToArchive) {
                          onAppendToArchive(
                            targetArchiveId,
                            isBulk ? selectedItemIds : [draggedId],
                          );
                          return;
                        }
                      }
              ...
              <PrimaryViewerCard
                activeNode={
                  ...
                  !(
                    ...
                    currentActiveNode.type === "archive_folder" ||
                    currentActiveNode.title?.toLowerCase().includes("archive")
                  )
                    ? currentActiveNode
                    : activeFolder &&
                        !(
                          ...
                          activeFolder.title?.toLowerCase().includes("archive")
                        )
                      ? {
                          ...activeFolder,
                          relation: "Folder",
              ...
              {isExpanded && (
                <motion.button
                  ...
                  onClick={(e) => {
                    ...
                    if (drillDownPath.length > 1 && onDrillUp) {
                      onDrillUp();

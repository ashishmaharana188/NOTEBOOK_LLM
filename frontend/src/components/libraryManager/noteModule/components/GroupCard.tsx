...
import type { NoteGroup, NoteItem } from "../../../../hooks/noteManager/useNotes";
            ...
            >
              ...
              <div className="w-full flex justify-between items-end border-b-2 border-gray-300 pb-3 mb-4 shrink-0">
                {isRenaming ? (
                  <div className="flex items-center gap-2 w-full pr-4">
                    <input
                      ...
                      className="flex-1 p-1 text-lg font-bold border border-gray-400 rounded bg-white"
                    />
                    <button
                      onClick={() => {
                        setIsRenaming(false);
                  ...
                  <div className="flex items-center gap-2 w-full pr-4 overflow-hidden group/title">
                    <h2
                      className="text-xl font-bold text-primary truncate cursor-pointer hover:text-blue-600 transition-colors"
                      onClick={() => {
                        setIsRenaming(true);
                        setEditTitle(group.title);
                      }}
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-accent text-accent-text text-xs font-bold rounded-md hover:bg-accent-hover transition shadow-sm whitespace-nowrap"
                >
                  + New Note
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2 pb-4 hide-scrollbar">
                {isActive && notes.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-32 text-gray-400 italic text-sm font-mono border-2 border-dashed border-border-subtle rounded-lg">
                    [ NO NOTES SAVED YET ]
                  </div>
                )}
                {isActive && notes.length > 0 && (
                  <div className="flex flex-col shrink-0 border-t-2 border-b-2 border-gray-900">
                    {notes.map((note, index) => (
                      ...
                      >
                        <h4 className="font-bold text-primary leading-tight truncate pr-4">
                          {note.title}
                        </h4>

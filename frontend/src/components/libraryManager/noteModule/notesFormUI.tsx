...
export default function NotesFormUI({
  ...
  const [title, setTitle] = useState(initialNote?.title || prefillTitle || "");
  ...
  const uploadFile = async (file: File) => {
    ...
    const response = await fetch(
      `http://localhost:8000/upload/media/note/${localNoteId.current}`,
      {
        method: "POST",
        body: formData,
      },
  ...
  useEffect(() => {
    ...
    const visibleLegacyTags = legacyTags.filter((tag) => !isSystemCanvasTag(tag));

    const stickyTexts = parsedStickies.map((s: any) => s.text).filter(Boolean);
  ...
  const executeSave = async (finalGroupId: string) => {
    ...
    onSave(title, contentHTML, finalTags, initialNote?.note_id, finalGroupId);
  };
          ...
          <div className="absolute inset-0 z-[10000] bg-slate-900/40 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-md p-6 flex flex-col">
              ...
              <select
                ...
                value={filingGroupId}
                onChange={(e) => setFilingGroupId(e.target.value)}
                disabled={!filingStackId}
              >
                <option value="">Select a Folder...</option>
                {groups
                  .filter(
                    (g: any) => String(g.stack_id) === String(filingStackId),
                  )
                  .map((g: any) => (

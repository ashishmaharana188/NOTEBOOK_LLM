...
export default function EchoDashboardUI(props: any) {
    ...
    const visibleEchoClusters = React.useMemo(
        () =>
            echoClusters.filter((cluster: any) => {
                if (
                    String(state.zoomTarget || "") === String(cluster.id || "")
                ) {
                    return true;
                }
    ...
    const visibleDraftBranchColumns = React.useMemo(
        () =>
            draftBranches.filter((draft: any) => {
                ...
                const position = state.positions[draft.id] || {
                    x: 1000,
                    y: 100,
                };
                return isRectVisible({
    ...
    const visibleDerivedColumns = React.useMemo(
        () =>
            (state.visibleDerivedColumns || []).filter((derived: any) => {
                if (
                    String(state.zoomTarget || "") === String(derived.id || "")
                ) {
                    return true;
                }
                const position = state.positions[derived.id] || {
                    x: 1180,
                    y: 100,
                };
                return isRectVisible({
    ...
    const visibleEchoEdges = React.useMemo(() => {
        const branchColumns = [
            ...echoClusters.map((cluster: any) => ({
                ...
                childId: String(cluster.id || ""),
                parentId: String(cluster.parent_cluster_id || ""),
                mode: String(cluster.column_metadata?.mode || ""),
                columnKind: String(cluster.column_metadata?.column_kind || ""),
                edgeStyle:
                    cluster.column_metadata?.mode ||
                    cluster.column_metadata?.column_kind === "analysis" ||
                    cluster.column_metadata?.column_kind === "rag"
                        ? "solid"
                        : "dashed",
            })),
            ...draftBranches.map((draft: any) => ({
                id: String(draft.id),
                childId: String(draft.id || ""),
        ...
        const persistedSourceEdges = echoClusters.flatMap((cluster: any) =>
            (cluster.column_metadata?.source_anchor_ids || [])
                .map((anchorId: string) => ({
                    id: `saved-edge:${anchorId}:${cluster.id}`,
                    parentId: String(anchorId || ""),
                    childId: String(cluster.id || ""),
                    mode: String(cluster.column_metadata?.mode || ""),
                    columnKind: String(
                        cluster.column_metadata?.column_kind || "",
                    ),
                    edgeStyle: "secondary",
                }))
                .filter(
        ...
        return [...branchColumns, ...persistedSourceEdges].filter(
            (cluster: any) => {
                if (!cluster.parentId) return false;
                const start = state.positions[cluster.parentId];
                const end = state.positions[cluster.childId || cluster.id];
                if (!start || !end) return false;
                if (
                    visibleEchoClusterIds.has(
                        String(cluster.childId || cluster.id),
                    ) ||
                ...
                return isRectVisible({
                    ...
                    y: edgeTop,
                    width:
                        Math.abs(end.x - (start.x + SAVED_COLUMN_WIDTH)) || 1,
            ...
            (visibleDerivedColumns || []).flatMap((derived: any) =>
                (derived.sourceAnchorIds || [])
                    ...
                    .map((anchorId: string) => ({
                        ...
                        columnKind: String(derived.columnKind || ""),
                        edgeStyle: "solid",
                    }))
                    .filter((edge: any) => {
                        ...
                        return isRectVisible({
                            ...
                            width:
                                Math.abs(end.x - start.x) + DRAFT_COLUMN_WIDTH,
                            height: Math.abs(end.y - start.y) + 160,
                        });
                    }),
            ),
    ...
    const buildContextFromChunk = React.useCallback(
        (
            ...
            full_text: String(
                chunk.full_text || chunk.text || chunk.bridge || "",
            ),
        }),
        [],
    );
                ...
                selectionRefs: [
                    {
                        kind: "incoming_echo",
                        id: String(
                            chunk.echo_id || chunk.chunk_id || chunkIndex,
                        ),
                        label:
                            chunk.title ||
                            group.title ||
            ...
            contexts: [
                {
                    ...
                    chapter:
                        derived.modeLabel || derived.mode || "Derived Analysis",
        ...
        async ({
            ...
            await state.createDraftBranchFromHighlight({
                ...
                bookId:
                    ...
                    activeBookTitle,
                libraryId:
                    item?.library_id || parentCluster.library_id || libraryId,
                spawnBasePosition:
                    state.positions[derived.id] || activeColumnPos,
            });
        },
        [
            activeBookTitle,
                                ...
                                >
                                    ...
                                    >
                                        <g transform="translate(15000, 15000)">
                                            {[
                                                ...
                                                const start =
                                                    state.positions[c.parentId];
                                                const childId =
                                                    c.childId || c.id;
                                                const end =
                                                    state.positions[childId];
                                                if (!start || !end) return null;
                                                const edgePresentation =
                                                    getEdgePresentation(c);
                                                const startX =
                                                    start.x +
                                                    getColumnWidthForId(
                                                        c.parentId,
                                                ...
                                                return (
                                                    <path
                                                        ...
                                                        strokeDasharray={
                                                            edgePresentation.strokeDasharray
                                                        }
                                                    />
                                                );
                                            })}
                                        ...
                                        (draft: any) => (
                                            <DraftBranchColumn
                                                ...
                                                ensureDraftBranchCluster={
                                                    state.ensureDraftBranchCluster
                                                }
                                                handleDraftBranchSaved={
                                                    state.handleDraftBranchSaved
                                                }
                                                closeDraftBranch={
                                                    state.closeDraftBranch
                                                }
                                                onCreateBranchFromHighlight={
                                                    state.createDraftBranchFromHighlight
                                                }
                                    ...
                                    {visibleDerivedColumns.map(
                                        (derived: any) => (
                                            <DerivedAnalysisColumn
                                                ...
                                                closeDerivedColumn={
                                                    state.closeDerivedColumn
                                                }
                                                saveDerivedColumn={
                                                    state.saveDerivedColumn
                                                }
                                                selectionMode={
                                                    state.selectionMode
                                                }
                                    ...
                                    {visibleEchoClusters.map((cluster: any) => (
                                        <SavedClusterColumn
                                            ...
                                            initialPos={
                                                state.positions[cluster.id] || {
                                                    x: 1000,
                                                    y: 100,
                                                }
                                            }
                                            zIndex={
                                                state.zIndexes[cluster.id] || 1
                                            }
                                            bringToFront={state.bringToFront}
                                            canvasScale={canvasScale}
                                            handleToggleActive={
                                            ...
                                            handleRenameCluster={
                                                state.handleRenameCluster
                                            }
                                            handleDeleteCluster={
                                                state.handleDeleteCluster
                                            }
                                            interactionReduced={isInteracting}
                                            expandedEchoId={
                                                state.expandedEchoByCluster?.[
                                                    cluster.id
                                                ] || null
                                            }
                                            onToggleEchoExpand={
                                                state.toggleSavedEchoExpansion
                                            }
                                            branchCountByEchoId={
                                                branchCountByEchoId
                                            }
                                            ...
                                            onShowBranches={
                                                state.focusBranchesForEcho
                                            }
                                            isHighlighted={state.highlightedBranchClusterIds?.has?.(
                                                String(cluster.id),
                                            )}
                                            originContext={state.resolveClusterOriginContext(
                                                cluster,
                                            )}
                                            selectionMode={state.selectionMode}
                                            isColumnSelected={state.isCanvasItemSelected(

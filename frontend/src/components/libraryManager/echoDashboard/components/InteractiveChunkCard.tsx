...
const InteractiveChunkCard = React.memo(
  ({
      ...
      try {
        if (isSaved && echoId) {
          const res = await axios.put(
            "http://127.0.0.1:8000/brain/echo/update_title",
            {
              echo_id: echoId,
          ...
          const res = await ensureRolesThen(["embedding"], () =>
            axios.post("http://127.0.0.1:8000/brain/echo/save", {
    ...
    const handleDelete = async () => {
      ...
      try {
        if (isSaved && echoId) {
          await axios.post("http://127.0.0.1:8000/brain/echo/delete", {
            echo_id: echoId,
          });
          setIsSaved(false);
          setEchoId(null);

  ...
  ({
    ...
    const getSelectionWithinContext = useCallback(() => {
      ...
      const marker =
        createMarkerFromSelection(
          ...
          String(chunk.text || ""),
          "excerpt",
        ) || createMarkerFromQuote(String(chunk.text || ""), text, "excerpt");
    ...
    const handleSelectionToggle = useCallback(
      (event: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (
          target?.closest(
            "button, input, textarea, a, [data-selection-ignore='true']",
          )
        ) {
          return;
        }
    ...
    const handleSaveAndReturnRef = async () => {
      ...
      try {
        if (isSaved && echoId) {
          const res = await axios.put(buildApiUrl("/brain/echo/update_title"), {
            echo_id: echoId,
          ...
          if (res.data.status === "success") {
            ...
            return {
              echoId,
              clusterId:
                savedClusterIdRef.current || String(res.data.cluster_id || ""),
            };
          }
          return null;
        }
    ...
    const handleOpenNoteManager = async () => {
      let currentEchoRef =
        echoId && (savedClusterIdRef.current || targetClusterId)
          ? {
              echoId,
              clusterId: savedClusterIdRef.current || targetClusterId || "",
            }
          : null;
      if (!isSaved || !currentEchoRef)
        currentEchoRef = await handleSaveAndReturnRef();
    ...
    const handleSearchHighlight = async () => {
      const nextSelection = selectionText.trim() || getSelectionWithinContext();
    ...
    const handleOpenReader = () => {
      onOpenReader?.({
        chunk,
        echoId: echoId || String((chunk as any).echo_id || ""),
        clusterId:
          savedClusterIdRef.current ||
          targetClusterId ||
          String((chunk as any).cluster_id || ""),

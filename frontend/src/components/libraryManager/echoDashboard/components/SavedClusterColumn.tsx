  ...
  ({
    ...
    return (
      <DraggableColumn
        ...
        onMaximize={() => onMaximize?.(cluster.id)}
        defaultWidth={470}
        defaultHeight={780}
              ...
              orderedChunks.map((chunk: any) => {
                ...
                return (
                  <SavedEchoCard
                    ...
                    branchCount={branchCountByEchoId[echoId] || 0}
                    onShowBranches={() => onShowBranches(echoId)}
                    onCreateBranchFromHighlight={(text) =>
                      onCreateBranchFromHighlight({
                        ...
                        libraryId: cluster.library_id || "",
                        originContext: {
                          title: chunk.title || cluster.title || "Source Echo",
                          text,
                          chapter: chunk.chapter || "Unknown Chapter",
                          source_label: chunk.filename || cluster.title || "",
                          echo_id: echoId,
                          cluster_id: cluster.id,
                          book_id: cluster.book_id || cluster.title,
                          library_id: cluster.library_id || "",
                          filename: String(chunk.filename || ""),
                          chunk_id: String(chunk.chunk_id || ""),
                          chunk_ref: String(chunk.chunk_ref || ""),
                          source_lid: String(chunk.source_lid || ""),
                        },
                      })
                    }
                    onAskRagFromHighlight={onAskRagFromHighlight}
                    onOpenReader={onOpenEchoReader}
                    selectionMode={selectionMode}
                    isSelected={isEchoSelected?.(echoId)}
                    onToggleSelect={() => onToggleEchoSelect?.(chunk)}
                  />
                );
              })
            )}

...
export default function useEchoDashboardState({
  ...
  const handleRenameCluster = useCallback(
    async (clusterId: string, newTitle: string) => {
      try {
        await axios.put("http://127.0.0.1:8000/brain/cluster/update_title", {
  ...
  const handleDeleteCluster = useCallback(async (clusterId: string) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8000/brain/cluster/${clusterId}`);
      await refreshCanvasSnapshot();
    } catch (err) {
  ...
  const handleToggleActive = async (
    ...
    try {
      await axios.post("http://127.0.0.1:8000/brain/cluster/activate", {
        cluster_id: clusterId,
        book_id: bookId,
        library_id: libId || "",
      });
  ...
  const handleSpawnCluster = async (
    ...
    try {
      const res = await axios.post(
        "http://127.0.0.1:8000/brain/cluster/spawn",
        {
          book_id: bookId,

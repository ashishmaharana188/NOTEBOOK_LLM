  ...
  const handleVectorize = async () => {
    ...
    try {
      ...
      if (res.data.status === "started") {
        setVectorizeMsg("✅ Task started in background! You can keep reading.");
      }
    } catch (e) {
      ...
      <div className="bg-surface p-6 rounded-sm shadow-sm border border-border-subtle">
        ...
        <div className="flex flex-wrap items-center justify-between mb-6 pb-6 border-b border-slate-100 gap-4">
          <div className="flex gap-2">
            ...
            >
              <MagnifyingGlassIcon className="w-3.5 h-3.5" /> Manual Query
            </button>
            <button
          ...
          <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 px-2 py-1 rounded-sm bg-canvas border border-border-subtle">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[8px] font-mono uppercase tracking-widest text-slate-500">
                    {vectorizing ? "Vectorizing" : "Last Run"}
                  </div>
                </div>
              </div>
            )}
            ...
            >
              {vectorizing ? (
                <>
                  <StopIcon className="w-3 h-3 animate-pulse" /> Stop Build
                </>
              ) : (
                <>
                  <CpuChipIcon className="w-3 h-3" /> Build Index
                </>
              )}
            </button>
          ...
          {mode === "SEARCH" ? (
            <>
              <div className="flex bg-slate-100 p-0.5 rounded-sm border border-border-subtle">
                <button
                  onClick={() => setSearchSource("InternetArchive")}
            ...
            <>
              ...
              <div className="flex-1 relative min-w-[200px]">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={curatorTopic}
                    onChange={(e) => setCuratorTopic(e.target.value)}
                    onKeyDown={(e) =>
                      e.key === "Enter" && handleCuratorSearch(curatorTopic)
                    }
                  />
                </div>
              </div>
              ...
              <div className="pb-0.5">
                <button
                  onClick={() => handleCuratorSearch(curatorTopic)}
                  disabled={loading}
        ...
        {searchMessage && (
          <div className="bg-canvas p-3 text-center border border-border-subtle mt-4 mx-6 rounded-sm">
            <p className="text-[10px] text-slate-600 font-mono font-bold uppercase tracking-widest flex items-center justify-center gap-2">
              {mode === "CURATOR" && <SparklesIcon className="w-3 h-3" />}{" "}
              {searchMessage}
            </p>
          </div>
        )}
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredResults.map((book) => (
            ...
            >
              ...
              <div className="p-4 flex gap-4 flex-1 mt-6">
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  ...
                  <p className="text-[10px] font-mono text-muted mb-2 truncate uppercase">
                    {book.author || "Unknown"} • {book.year || "N/A"}
                  </p>
              ...
              >
                <button
                  onClick={() => triggerDownload(book, downloadFormat)}
                ...
                >
                  <ArrowDownTrayIcon className="w-3 h-3" /> Fetch
                </button>
                <button
                  onClick={() => handleDirectIngest(book, downloadFormat)}

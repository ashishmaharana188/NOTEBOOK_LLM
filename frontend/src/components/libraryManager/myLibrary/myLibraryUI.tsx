...
const normalizeFileKey = (value: string) =>
  value.toLowerCase().replace(/\.[^/.]+$/, "").replace(/[\s_-]+/g, "");
...
export default function MyLibrary({
  ...
  const queuedFilenames = useMemo(
    () =>
      new Set(
        (ingestQueue?.queued || []).map((job) => job.filename).filter(Boolean),
      ),
    [ingestQueue],
  );
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedFiles.size > 0 ? (
              <div className="flex items-center gap-3 animate-in fade-in">
                ...
                >
                  <CpuChipIcon className="w-3 h-3" /> Add to Brain
                </button>
                <button
              ...
              <div className="flex items-center gap-4">
                ...
                {ingestQueue && ingestQueue.counts.total > 0 && (
                  <>
        ...
        <div className="divide-y divide-slate-100">
          {filteredFiles.length === 0 ? (
            ...
            filteredFiles.map((filename) => {
              ...
              const normalizedFilename = normalizeFileKey(filename);
              ...
              const statusBits = [
                ...
                isCancelling ? "Cancelling" : isIngesting ? "Ingesting" : null,
                  ...
                  >
                    {isIngesting || isQueued ? (
                      <>
                        ...
                        <button
                          onClick={() => onCancelIngest(filename)}

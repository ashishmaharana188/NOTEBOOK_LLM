...
const InspectorPanel = ({
  ...
  return (
    <div
      ref={panelRef}
      className="absolute left-3 right-3 top-16 z-40 flex max-h-[calc(100%-1.5rem)] flex-col overflow-y-auto rounded-none border border-gray-300 bg-surface/95 shadow-2xl  -sm sm:left-auto sm:right-6 sm:top-6 sm:max-h-[90%] sm:w-80"
    >
      <div className="p-5 pb-4 relative border-b border-border-subtle">
        ...
        {selectedNode.group === "stacks" && (
          <div className="flex flex-col gap-2 mt-4">
            ...
            <button
              onClick={() =>
                navigateToNotes("VIEW_STACK", {
                  stackId: selectedNode.id,
                })
              }
              className="w-full py-2 bg-gray-900 hover:bg-accent-hover text-white text-[10px] font-bold uppercase tracking-widest transition-colors shadow-sm"
            >
